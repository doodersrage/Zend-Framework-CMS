...
class Sc_Action_Helper_Mobile extends Zend_Controller_Action_Helper_Abstract
{
	public $view;
...
//			}
			...
			$view->copy_text = str_replace(array('<div class="formpad">&nbsp;</div>','<div class="clear">&nbsp;</div>'),array('<div class="formpad"></div>','<div class="clear"></div>'),$pages->getMobile_text());
			if($pages->getTitle_tag() != ''){
				$view->headTitle()->prepend($pages->getTitle());
			}
			if($pages->getDesc_tag() != ''){
				$view->headMeta()->prependName('description', $pages->getDesc_tag());	
			}
			if($pages->getKeyword_tag() != ''){
				$view->headMeta()->prependName('keywords', $pages->getKeyword_tag());
			}
		}
    }
